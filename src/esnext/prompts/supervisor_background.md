...
Inspect artifacts or logs first, then decide whether to wait, resume the worker with concrete findings, or schedule a later resume.
...
Do not cancel a background worker just because it is waiting for future progress; cancel only if it is clearly no longer useful.
